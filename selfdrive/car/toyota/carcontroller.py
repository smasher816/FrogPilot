  ...
  def update(self, CC, CS, now_nanos, frogpilot_toggles):
    ...
    if self.CP.carFingerprint != CAR.TOYOTA_PRIUS_V:
      ...
      if self.frame % 20 == 0 or send_ui:
        always_on_lateral_enabled = frogpilot_toggles.always_on_lateral_main or CS.out.cruiseState.enabled
        always_on_lateral_enabled &= frogpilot_toggles.always_on_lateral
        always_on_lateral_enabled &= not frogpilot_toggles.always_on_lateral_lkas
        can_sends.append(toyotacan.create_ui_command(self.packer, steer_alert, pcm_cancel_cmd, hud_control.leftLaneVisible,
                                                     hud_control.rightLaneVisible, hud_control.leftLaneDepart,
                                                     hud_control.rightLaneDepart, CC.enabled, CS.lkas_hud, always_on_lateral_enabled, lat_active))
